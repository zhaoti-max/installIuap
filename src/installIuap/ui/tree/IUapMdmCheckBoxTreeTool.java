...
import installIuap.common.base.IUapMdmTreeDataBean;
 ...
 * 
 */
public class IUapMdmCheckBoxTreeTool {
	...
	public IUapMdmCheckBoxTreeTool(IVOTreeDataByCode voTreeData, String rootName) {
	...
	public List<String> getCheckedLeafNode() {
		...
		IUapMdmTreeDataBean nodeBean = null;
		Object obj = null;
		...
		for (int idx = 0; idx < node.length; idx++) {
			if (node[idx].isLeaf()) {
				...
				if ((obj instanceof IUapMdmTreeDataBean)) {
					nodeBean = (IUapMdmTreeDataBean) obj;
					codeLst.add(nodeBean.getCode());
				}
			}
		}
	...
	class TreeModelCreatorByCodeRule {
		...
		public DefaultTreeModel createTree() {
			return createTree(IUapMdmCheckBoxTreeTool.this.voTreeDataByCode);
		}
		...
		public DefaultTreeModel createTree(Object[] objs) {
			try {
				if ((objs != null) && (objs.length > 0)) {
					...
					return createTree(nodes, new CheckTreeNode(IUapMdmCheckBoxTreeTool.this.rootName));
				}
			} catch (Exception e) {
